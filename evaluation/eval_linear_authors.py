...
def evaluation_forward(x, model, conv):
    if model.sobel:
        x = model.sobel(x)
    count = 1
...
def eval_linear(model: DeepClusteringNet, conv_layer, train_dataset, val_dataset,
                loss_function, random_state=0, **kwargs):
    """ A method to evalute model performance

    Args:
        model (DeepClusteringNet): [description]
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset,
                                               ...
                                               shuffle= True,
                                               num_workers= kwargs.get("training_workers", 4),
                                               pin_memory=True
                                               )

    val_loader = torch.utils.data.DataLoader(val_dataset,
                                             batch_size=kwargs.get("validation_batch_size", 128),
                                             shuffle=False,
                                             num_workers=kwargs.get("validation_workers", 4)
                                             )


    # Define Logistic Regression Layer
    reglog = RegLog(conv_layer, len(train_dataset.classes)).to(model.device)
...
def train(train_loader, model, reglog, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        learning_rate_decay(optimizer, len(train_loader) * epoch + i, optimizer.lr)
