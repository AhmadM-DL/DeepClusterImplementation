...
def run(device, batch_norm, lr, wd, momentum, n_cycles,
    ...
    cifar100 = CIFAR100(dataset_path, download=True)
    ...
    dataset = DeepClusteringDataset(cifar100)
    ...
    writer_file = "cifar100_microAlexNet_"+"batchnorm(%d)_" % batch_norm + \
    ...
    cifar100_test = CIFAR100(dataset_path, train=False, download=True)

    traindataset = cifar100
    validdataset = cifar100_test
    ...
    cifar100.transform = transforms.Compose(transformations_train)
    cifar100_test.transform = transforms.Compose(transformations_val)
