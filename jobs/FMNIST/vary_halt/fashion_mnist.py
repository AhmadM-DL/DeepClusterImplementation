...
def run(device, batch_norm, lr, wd, momentum, n_cycles,
    ...
    writer_file = "{dataset}_{model}_batchnorm({bt})_lr({lr})_momentum({mom})_wdecay({wd})_n_clusters({nclusters})_n_cycles({ncycles})_rnd({seed})_t_batch_size({tbsize})_shuffle({shfl})_halt({halt})_"
    writer_file = writer_file.format(dataset=DATASET, model=MODEL, bt=batch_norm, lr=lr, mom=momentum,
                                     ...
                                     tbsize=training_batch_size, shfl=training_shuffle, halt=halt)
    if pca:
